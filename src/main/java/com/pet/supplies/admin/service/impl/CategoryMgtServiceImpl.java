...
public class CategoryMgtServiceImpl implements CategoryMgtService {
	...
	public CategoryModel saveCategory(CategoryModel model) {
		...
		if (model != null) {
			...
			Category category = ModelToEntityMapper
					.mapCategoryModelToCategoryEntity(model);
			newCategory = categoryRepository.save(category);
			categoryModel = EntityToModelMapper
					.mapCategoryEntityToCategoryModel(newCategory);
		}
